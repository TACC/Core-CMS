...
- **One plugin:** [`HeaderLogoPlugin`](taccsite_cms/contrib/taccsite_header_logo/cms_plugins.py) extends Picture (same model, no migration). Toolbar: **TACC Header → Header logo** — image + link, no template dropdown.
- **Render:** CMS renders plugin at `{% static_placeholder "header-content" %}`; markup via [`header_logo_markup.html`](taccsite_cms/templates/header_logo_markup.html).
